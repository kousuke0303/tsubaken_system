class Employees::ConstructionReportsController < Employees::EmployeesController
  ...
  def index
    if params[:type] == "uncheck"
      ...
      construction_reports = ConstructionReport.includes(construction_schedule: :matter).where(work_date: params[:work_date])
  ...
  def confirmation
    ...
    if current_admin
      @construction_reports.update_all(admin_check: true)
    end
  ...
  def update
    ...
    if @construction_report.update(update_params.merge(sm_check: false, admin_check: false))
      ...
      redirect_to supplier_managers_construction_reports_path
    end
  end
