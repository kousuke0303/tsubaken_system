class Employees::EstimateMatters::MembersController < Employees::EmployeesController
  ...
  def member_change_for_supplier_staff
    ActiveRecord::Base.transaction do
      # 既存の外注企業スタッフ消去
      @supplier = Supplier.find(params[:estimate_matter][:supplier_id])
      supplier_staff_codes_ids = @supplier.external_staffs.ids
      registed_staff_code_ids = @estimate_matter.member_codes.where(external_staff_id: supplier_staff_codes_ids).ids
      delete_matter_member_codes = EstimateMatterMemberCode.where(estimate_matter_id: @estimate_matter.id, member_code_id: registed_staff_code_ids) 
      delete_matter_member_codes.delete_all
      # 新規スタッフ登録
      params[:estimate_matter][:member_code_ids].each do |supplier_staff_code_id|
        @estimate_matter.estimate_matter_member_codes.create(member_code_id: supplier_staff_code_id)
      end
    end
