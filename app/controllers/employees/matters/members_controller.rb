class Employees::Matters::MembersController < Employees::EmployeesController
  ...
  def member_change_for_vendor_staff
    ActiveRecord::Base.transaction do
      # 既存の外注企業スタッフ消去
      @vendor = Vendor.find(params[:matter][:vendor_id])
      vendor_staff_codes_ids = @vendor.external_staffs.ids
      registed_staff_code_ids = @matter.member_codes.where(external_staff_id: vendor_staff_codes_ids).ids
      delete_matter_member_codes = MatterMemberCode.where(matter_id: @matter.id, member_code_id: registed_staff_code_ids)
      delete_matter_member_codes.delete_all
      # 新規スタッフ登録
      params[:matter][:member_code_ids].each do |vendor_staff_code_id|
        @matter.matter_member_codes.create(member_code_id: vendor_staff_code_id)
      end
    end
